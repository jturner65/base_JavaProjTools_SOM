...
public class SOM_GeomProjConfig extends SOM_ProjConfigData {
	...
	protected void _loadIndivConfigVarsPriv(String varName, String val) {
		switch (varName) {
		default : {
			msgObj.dispMessage("SOM_GeomProjConfig","_loadIndivConfigVarsPriv","Unknown/unhandled config var name :  " +varName +" with value " + val +" .", MsgCodes.warning2);
			}
		}
	...
	protected ArrayList<String> buildCustSOMConfigData() {
		...
		customData.add("# This file holds Geom project-specific configuration data pertaining to the training of the SOM in this directory");
	...
	protected void setSOM_UsePreBuilt_Indiv() {
		...
		while (idx < fileStrings.length) {
			...
			switch (varName) {		
				default : {
					msgObj.dispMessage("SOM_GeomProjConfig","setSOM_UsePreBuilt_Indiv","Unknown/unhandled custom config var name :  " +varName +" with value " + val +" .", MsgCodes.warning2);
				}
			}//switch
			++idx;
		}//while
	...
	protected String getSegmentFileNamePrefix_Indiv(String segType, String fNamePrefix) {
		...
		default : {
			msgObj.dispMessage("SOM_GeomProjConfig","getSegmentFileNamePrefix_Indiv","Unknown Segment Type " +segType.toLowerCase() +" so unable to build appropriate file name prefix.  Aborting.", MsgCodes.warning2);
			return "";	}
		}
